...
// Core
function getKeyIndex(key: Note): number {
...
function getModeInterval(mode: Mode, withOctave: boolean = false): Array<Interval> {
  let modeInterval = MODE_INTERVAL_MAP[mode]
  if (withOctave)
    modeInterval = modeInterval.concat(OCTAVE_INTERVAL)
  return modeInterval
}
function getModeSemitone(mode: Mode, withOctave: boolean = false): number[] {
...
function getScale(key: Note, mode: Mode, isSharp: boolean = true, withOctave: boolean = false): SingleNoteList {
...
function getRelativeMinorKey(majorKey: Note, isSharp: boolean = true): Note {
...
function getCircleOfFifthList(isSharp: boolean = true): CircleOfFifthList {
  let times = 12
  let currentIndex = 0
  const list = []
  while (times > 0) {
...
function getNoteListByStep(startNote: Note, interval: Interval, amount: number, isSharp: boolean = true): SingleNoteList {
